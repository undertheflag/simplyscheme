...
(define (n-card-poker hand wild)
  (cdr (max (lambda (a b) (< (car a) (car b))) (every3 (lambda (x) (five-card-poker x wild)) (make-every-combo hand 5)))))

(define (five-card-poker hand wild)
...
(define (suit2 hand wild)
  (let ((cards (first (sort-wild hand wild))))
    (cond ((null? (cdr cards)) #t)
          ((equal? (first (first cards)) (first (first (butfirst cards)))) (suit2 (butfirst cards) '-))
          (else #f))))

(define (rank-info2 cards wild)
  (let* ((sw (sort-wild cards wild))
         (r-i (rank-info (first sw))))
    (rank-info2-help r-i (last sw) (max > r-i))))

(define (rank-info2-help hand-info num-of-wilds max-hand-info)
  (cond ((equal? (first hand-info) max-hand-info) (cons (+ num-of-wilds (first hand-info)) (butfirst hand-info)))
        (else (cons (first hand-info) (rank-info2-help (butfirst hand-info) num-of-wilds max-hand-info)))))
...
(define (longest-match pattern-rest sent min max-one?)
  (cond ((empty? sent) (cond ((and (= min 0)
...
(define (lm-helper pattern-rest sent-matched sent-unmatched min)
  (cond ((< (length sent-matched) min) #f)
        ((match? pattern-rest sent-unmatched) sent-matched)
        ((empty? sent-matched) #f)
...
;(define (match-using-known-values-2 pattern sent known-values)
 ; (cond ;((and (empty? pattern) 
         ;     (empty? sent)) known-values)
        ;((empty? pattern) '(failed))
        ;((member? (butfirst (first pattern)) known-values) (cond ((is-it-the-beginning? (look-up (butfirst (first pattern)) known-values) sent)(match-using-known-values (butfirst pattern) (remove-from-beginning (look-up (butfirst (first pattern)) known-values) sent) known-values))
         ;                                                        (else '(failed))))
  ;      ((and (match? (convert-to-match?-form pattern) sent)
   ;           (special? (first (first pattern)))) 
    ;     (match-using-known-values (butfirst pattern) (remove-from-beginning (match-special (first (first pattern)) (butfirst (convert-to-match?-form pattern)) sent) sent) (append (list (butfirst (first pattern))) (matching-text pattern sent) '(!) known-values)) )
     ;   ((equal? (first pattern) (first sent)) (match-using-known-values (butfirst pattern) (butfirst sent) known-values))
      ;  (else '(failed))))
...
(define (match-using-known-values pattern sent known-values)
  (cond ((and (empty? pattern)
              (empty? sent)) known-values)
        ((empty? pattern) '(failed))
        ((look-up-2 (first pattern) known-values) (cond ((match-existing? pattern sent known-values))
                                                        (else '(failed))))
        ((and (match? (convert-to-match?-form pattern) sent)
              (special? (first (first pattern)))) (data-entry pattern sent known-values))
        ((equal? (first pattern) (first sent)) (match-using-known-values (butfirst pattern) (butfirst sent) known-values))
        (else '(failed))))

(define (look-up-2 data known-values)
  (cond ((null? known-values) #f)
        ((equal? data (first (first known-values))) (last (first known-values)))
        (else (look-up-2 data (butfirst known-values)))))

(define (match-existing? pattern sent known-values)
  (let ((matching-text (look-up-2 (first pattern) known-values)))
  (cond ((beginning? matching-text sent) (match-using-known-values (butfirst pattern) (remove-2 matching-text sent) known-values))
        (else #f))))

(define (beginning? beg sent)
  (cond ((null? beg) #t)
        ((null? sent) #f)
        ((equal? (first beg) (first sent)) (beginning? (butfirst beg) (butfirst sent)))
        (else #f)))

(define (remove-2 beg sent)
  (cond ((null? beg) sent)
        (else (remove-2 (butfirst beg) (butfirst sent)))))

(define (data-entry pattern sent known-values)
  (vprint (data-entry-helper pattern sent known-values))
  (cond ((null? (filter (lambda (x) (not (equal? x '(failed)))) (apply append (data-entry-helper pattern sent known-values)))) '(failed))
        (else (first (filter (lambda (x) (not (equal? x '(failed)))) (apply append (data-entry-helper pattern sent known-values)))))))

(define (data-entry-helper pattern sent known-values)
  (let ((value (match-special (first (first pattern)) (convert-to-match?-form (butfirst pattern)) sent)))
  (filter (lambda (x) (list? (first x))) (map (lambda (v) (match-using-known-values (butfirst pattern) (remove-2 v sent) (append known-values (list (list (first pattern) v))))) (go-down value)))))

(define (go-down values)
  (cond ((null? values) '(()))
        (else (cons values (go-down (butlast values))))))

(define (convert-to-match?-form pattern)
  (cond ((empty? pattern) pattern)
        ((special? (first (first pattern))) (cons (first (first pattern)) (convert-to-match?-form (butfirst pattern))))
        (else (cons (first pattern) (convert-to-match?-form (butfirst pattern))))))

(define (matching-text pattern sent)
  (match-special (first (first pattern)) (butfirst (convert-to-match?-form pattern)) sent))

;(define (is-it-the-beginning? poss-beginning sent)
 ; (vprint (list poss-beginning sent))
  ;(cond ((empty? poss-beginning) #t)
   ;     ((equal? (first poss-beginning) (first sent)) (is-it-the-beginning? (butfirst poss-beginning) (butfirst sent)))
    ;    (else #f)))

;(define (look-up word known-values)
 ; (cond ((equal? word (first known-values)) (return-until-! (butfirst known-values)))
  ;      (else (look-up word (butfirst known-values)))))

;(define (return-until-! known-values)
 ; (cond ((equal? (first known-values) '!) '())
  ;      (else (cons (first known-values) (return-until-! (butfirst known-values))))))

;(define (remove-from-beginning beginning sent)
 ; (cond ((empty? beginning) sent)
  ;      (else (remove-from-beginning (butfirst beginning) (butfirst sent)))))

;17.13 ;change output to association list (list of lists)

;end of pattern matcher

;find bug in examples

;chapter 16 excercizes

;16.8: when given an empty sentence, the longest match program first checks to see if the minimum match for the placeholder is zero. If it is, it checks for a match. If not, it returns false

;16.9: pattern: '() sentence: '(something something)

;every~map keep~filter accumulate~reduce
;chapter 17: lists
...
(define (f4 lst1 lst2)
  (list (cons (car lst1) (cons (car lst2) ())) (append (cdr lst1) (cdr lst2))))

;17.3: a list of procedures

;17.4: it reverses the list
...
(define (max3 number . rest-of-numbers)
  (max3-helper (max2 number (car rest-of-numbers)) (cdr rest-of-numbers))) 

(define (max3-helper number lst-of-rest-numbers)
  (cond ((null? lst-of-rest-numbers) number)
        (else (max3-helper (max2 number (car lst-of-rest-numbers)) (cdr lst-of-rest-numbers)))))

;17.6

(define (append2 lst1 lst2)
  (append2-helper (reverse lst1) lst2))

(define (reverse lst)
  (reverse-helper lst '()))

(define (reverse-helper lst other)
  (if (null? lst)
      other
      (reverse-helper (cdr lst) (cons (car lst) other))))

(define (append2-helper lst1-rev lst2)
  (cond ((null? lst1-rev) lst2)
        (else (append2-helper (cdr lst1-rev) (cons (car lst1-rev) lst2)))))

(define (append3 lst . rest-of-lsts) ;this is the final append program
  (append3-helper (append2 lst (car rest-of-lsts)) (cdr rest-of-lsts)))

(define (append3-helper lst lst-of-rest-lsts)
  (cond ((null? lst-of-rest-lsts) lst)
        (else (append3-helper (append2 lst (car lst-of-rest-lsts)) (cdr lst-of-rest-lsts)))))

;17.7

(define (sentence2 thing1 thing2)
  (cond ((word? thing1) (if (word? thing2) (list thing1 thing2) (cons thing1 thing2)))
        (else (if (word? thing2) (append thing1 (list thing2)) (append thing1 thing2)))))

(define (sentence3 thing . rest-of-things)
  (sentence3-helper (sentence2 thing (car rest-of-things)) (cdr rest-of-things)))

(define (sentence3-helper lst lst-of-rest-things)
  (cond ((null? lst-of-rest-things) lst)
        (else (sentence3-helper (sentence2 lst (car lst-of-rest-things)) (cdr lst-of-rest-things)))))

;17.8

(define (member2 word lst)
  (cond ((null? lst) #f)
        ((equal? word (car lst)) lst)
        (else (member2 word (cdr lst)))))

;17.9

(define (list-ref2 lst number)
  (cond ((= number 0) (car lst))
        (else (list-ref (cdr lst) (- number 1)))))

;17.10

(define (length2 lst)
  (length2-helper lst 0))

(define (length2-helper lst counter)
  (cond ((null? lst) counter)
        (else (length2-helper (cdr lst) (+ 1 counter)))))

;17.11

(define (before-in-list? lst thing1 thing2)
  (cond ((equal? (car lst) thing1) (member? thing2 lst))
        ((equal? (car lst) thing2) #f)
        ((null? lst) #f)
        (else (before-in-list? (cdr lst) thing1 thing2))))
...
;19.1: (lennon mccartney harrison starr), map would give ((lennon) (mccartney) (harrison) (starr)) 
;every cadr, (e c a t), map cadr (e c a t)

;19.2

(define (keep2 cmp-fn thing)
  (cond ((word? (first thing))(keep2-helper cmp-fn thing word))
        (else (keep2-helper cmp-fn thing sentence))))

(define (keep2-helper cmp-fn thing combiner)
  (cond ((empty? thing) thing)
        (else (combiner (cmp-fn (first thing)) (keep2-helper cmp-fn (butfirst thing) combiner)))))

;19.3

(define (three-arg-accumulate fn result lst)
  (cond ((null? lst) result)
        (else (three-arg-accumulate fn (fn (last lst) result) (butlast lst)))))

;19.4

(define (left-accumulate fn lst)
  (cond ((null? (cdr lst)) (first lst))
        (else (fn (left-accumulate fn (butlast lst)) (last lst)))))

;19.5

(define (true-for-all?-2 pred lst)
  (cond ((null? lst) #t)
        ((pred (car lst)) (true-for-all?-2 pred (cdr lst)))
        (else #f)))

;19.6

(define (true-for-any-pair? fn lst)
  (true-for-any-pair?-helper fn lst '()))

(define (true-for-any-pair?-helper fn input output)
  (cond ((null? input) #t)
        ((null? (cdr input)) (cond ((null? output) #t)
                                   (else (check-for-t output))))
        ((fn (car input) (cadr input)) (true-for-any-pair?-helper fn (butfirst input) (cons '#t output)))
        (else (true-for-any-pair?-helper fn (butfirst input) (cons '#f output)))))

(define (check-for-t lst)
  (cond ((null? lst) #f)
        ((equal? (car lst) #t) #t)
        (else (check-for-t (cdr lst)))))
...
(define (true-for-all-pairs? fn lst)
  (cond ((null? lst) #t)
        ((null? (cdr lst)) #t)
        ((fn (car lst) (cadr lst)) (true-for-all-pairs? fn (cdr lst)))
        (else #f)))
...
(define (sort2 lst pred)
  (cond ((null? lst) lst)
        (else (place2 (car lst) (sort2 (cdr lst) pred) pred))))

(define (place2 thing lst pred)
  (cond ((null? lst) (cons thing lst))
        ((pred thing (car lst)) (cons thing lst))
        (else (cons (car lst) (place2 thing (cdr lst) pred)))))
...
(define (repeated2 num fn)
  (cond ((= num 0) (lambda (x) x))
        (else (lambda (x) ((repeated2 (- num 1) fn) (fn x))))))
